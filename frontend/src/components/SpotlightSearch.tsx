...
export default function SpotlightSearch({ onSelect }: SpotlightSearchProps) {
  ...
  return (
    <>
      ...
      {open ? (
        <div className="fixed inset-0 z-[60] flex items-start justify-center bg-slate-950/40 px-4 py-16 backdrop-blur-sm">
          <div className="w-full max-w-2xl rounded-[1.75rem] border border-border/70 bg-[var(--panel-strong)] p-5 shadow-[0_24px_80px_rgba(15,23,42,0.26)]">
            <div className="flex items-center gap-3 rounded-[1.25rem] border border-border/70 bg-card px-4 py-3">
              ...
              />
              <span className="text-[11px] font-black uppercase tracking-[0.18em] text-muted-foreground">Esc</span>
            </div>
            ...
            <div className="mt-4 space-y-2">
              ...
              {!isFetching && query.trim().length > 1 && !data?.results?.length ? (
                <div className="empty-card text-sm text-muted-foreground">No matching student, faculty, or subject was found.</div>
              ) : null}
            </div>
          </div>
        </div>
